...
class Apib2Json {
    ...
    toArray(apib) {
        return new Promise((resolve, reject) => {
            const data = {};
            Protagonist.parse(apib, {generateSourceMap: true}, (error, result) => {
                if (error) {
                    reject(error);
                    return;
                }

                const content = (typeof result.content[0].content == 'string') ? [] : result.content[0].content;
                            ...
                            transition.content.forEach((content) => {
                                ...
                                content.content.forEach((httpTransaction) => {
                                    ...
                                    httpTransaction.content.forEach((rContent) => {
                                        ...
                                        if (isAsset && rContent.attributes.contentType.content === 'application/schema+json') {
